...
//Полагаем, что нам требуется структура данных, содержащая заранее известный допустимый набор параметров 
//различного типа - строки, целые, числа с плавающей точкой и т.д., а также признак, является ли этот параметр 
//обязательным.
//Полагаем, что параметры могут передаваться только в "длинном" формате с обязательным "--" 
//перед именем параметра(то есть "--my-option-name my-option-val", "--my-option-name=my-option-val", 
//	"--my-boolean-option").
//Параметров может быть несколько.
//Функция должна обязательно уметь обрабатывать параметр "--help" (если он указан в списке параметров при 
//вызове функции), выводящий пример использования(необязательные параметры должны выводиться в квадратных скобках).
//Реализация требуется на С++ и / или Python.

//Варианты для тестирования:
//1) без help удачное завершение
//--my-boolean-option = true --my-option-name my-option-name --my-option-fval 0.7 --my-option-ival=11
//
//2) с help, возвращает 0
// --help
//--my-boolean-option true --help --my-option-name my-option-name --my-option-fval 0.7 --my-option-ival 11
//
//3)неудачное завершение, incorrect input
//--my-boolean-option sfd; --my-option-name = my-option-name --my-option-fval 0.7 --my-option-ival=11 --my-option-name tyu 
//
//4) неудачное завершение, ошибка при вводе команды --
//my-boolean-option sfd; --my-option-name my-option-name --my-option-fval =  0.7 --my-option-ival 11
//
//5) неудачное завершение, ошибка при вводе значения, не получается преобразовать в вид float  или int
//--my-boolean-option --my-option-name = my-option-name --my-option-fval=0.7 --my-option-ival=map

//6) неудачное завершение, ошибка при вводе значения, не получается преобразовать в вид float  или int
	...
	do
	{
		...
		for (i = 0; i < Rules.size(); ++i)
		{
			if (temp_str == Rules[i][1])
			{
				...
				{
					print(Rules);
					return 0;            //the function getopt return 0, the case of --help or empy input string
				}
				break;
			}
		}
...
//--my-boolean-option --my-option-name = my-option-name --my-option-fval=0.7 --my-option-ival=map
//6) неудачное завершение, ошибка при вводе значения, не получается преобразовать в вид float  или int
//--my-boolean-option false --my-option-name = my-option-name --my-option-fval=0.7 --my-option-ival a12f